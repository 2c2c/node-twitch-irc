...
import {ListGroup, FormControl, Modal, Button, ButtonGroup} from 'react-bootstrap';
...
export default class UserList extends Component {
  render() {
    ...
    let rendered_list = userlist.map((user, id) => {
      return (<UserElement key={name + user} user={user} focusedUser={focusedUser} handleFocusedUser={(user) => handleFocusedUser(user)}/>);
    });
    ...
    return (
      <div className={name}>
        <h4 style={{textWeight: 'bold', textAlign: 'center'}}>{name}</h4>
